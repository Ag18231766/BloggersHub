...
export function PostsRenderComponent({title}:OptionalProps) {
    ...
    const fetch = useCallback(async function () {
        try {
            ...
            if(title === undefined){
                response = await axios.get(`http://localhost:5000/api/v1/posts/yourposts/${count}`, {
                    headers: {
                        authheader: 'Bearer ' + localStorage.getItem('token')
                    }
                });     
            }else{
                response = await axios.get(`http://localhost:5000/api/v1/posts/allposts/?title=${title}&&count=${count}`, {
                    headers: {
                        authheader: 'Bearer ' + localStorage.getItem('token')
                    }
                });
    ...
    useEffect(() => {
        ...
        if(title === undefined){
            console.log('your');
            axios.get(`http://localhost:5000/api/v1/posts/yourposts/1`, {
                headers: {
                    authheader: 'Bearer ' + localStorage.getItem('token')
                }
            }).then((response) => {
            ...
            axios.get(`http://localhost:5000/api/v1/posts/allposts/?title=${title}&&count=1`, {
                headers: {
                    authheader: 'Bearer ' + localStorage.getItem('token')
                }
            }).then((response) => {
